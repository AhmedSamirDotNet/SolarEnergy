...
import { getProjectCards } from "@/lib/api";
import { Loader2, MapPin } from "lucide-react";

export function FeaturedProjects() {
  const { t, language } = useI18n();
  const [projects, setProjects] = useState<any[]>([]); // استعملنا any عشان مرونة الـ Mapping الجديد
  ...
  return (
    <section
      id="projects"
      className="relative overflow-hidden py-12 sm:py-16 md:py-20 lg:py-24 bg-gradient-to-b from-emerald-900/30 via-emerald-800/20 to-emerald-900/30 backdrop-blur-xl border-t border-white/10"
    >
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-10 sm:mb-12 md:mb-16">
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-foreground mb-3 sm:mb-4">
            {t("projects.title") ||
              (language === "en" ? "Featured Projects" : "مشاريعنا المميزة")}
          </h2>
        ...
        {loading ? (
          ...
          <div className="space-y-8 sm:space-y-10 md:space-y-12">
            {projects.map((project, index) => {
              ...
              // استخراج البيانات من الهيكل الجديد للباك-أند
              const title = project.translation?.title || "Untitled Project";
              const location = project.translation?.locationText;
              const imageUrl = project.imageUrl || "/placeholder-image.jpg";
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-12">
                    ...
                    >
                      <Image
                        ...
                        unoptimized // لأننا بنجيب رابط كامل من سيرفر خارجي (الباك-أند)
                      />
